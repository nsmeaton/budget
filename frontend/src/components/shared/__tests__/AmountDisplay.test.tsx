...
describe('AmountDisplay', () => {
  it('renders income amounts with + prefix and green colour', () => {
    renderWithPrivacy(<AmountDisplay amount={4500} direction="in" flowType="income" />)
  ...
  it('renders spending amounts with - prefix and red colour', () => {
    renderWithPrivacy(<AmountDisplay amount={-85.5} direction="out" flowType="spending" tier="Essential" />)
  ...
  it('renders neutral amounts (Savings tier) without sign prefix', () => {
    renderWithPrivacy(<AmountDisplay amount={-1000} direction="out" tier="Savings" />)
  ...
  it('renders Transfer tier as neutral', () => {
    renderWithPrivacy(<AmountDisplay amount={-500} direction="out" tier="Transfer" />)
  ...
  it('renders transfer flow_type as neutral', () => {
    renderWithPrivacy(<AmountDisplay amount={-200} direction="out" flowType="transfer" />)
  ...
  it('uses absolute value for display', () => {
    renderWithPrivacy(<AmountDisplay amount={-42.99} direction="out" flowType="spending" tier="Optional" />)
    expect(screen.getByText('-£42.99')).toBeInTheDocument()
  })

  it('formats zero amount correctly', () => {
    renderWithPrivacy(<AmountDisplay amount={0} direction="out" flowType="spending" />)
    // 0 is not income, not neutral, so should show as spending
    expect(screen.getByText('-£0.00')).toBeInTheDocument()
  })

  it('applies custom className', () => {
    renderWithPrivacy(<AmountDisplay amount={100} direction="in" flowType="income" className="text-lg" />)
